 ...
 *             // to `this` using {@link CLI#apply}. Further, the `listeners` property is
 ...
 *     });
 */
CLI.define('CLI.util.Observable', function(Observable) {

    var emptyFn = CLI.emptyFn,
        emptyArray = [],
    ...
    return {
        ...
        statics: {
            ...
            *
            * @param {CLI.util.Observable} o The Observable to release
            * @static
            */
            ...
            *
            * @param {CLI.util.Observable} o The Observable to capture events from.
            ...
            */
            ...
            captureArgs: function(o, fn, scope) {
                o.fireEventArgs = CLI.Function.createInterceptor(o.fireEventArgs, fn, scope);
            },
            ...
            * Usage:
            *
            *     CLI.util.Observable.observe(CLI.data.Connection);
            *     CLI.data.Connection.on('beforerequest', function(con, options) {
            ...
            * @param {CLI.util.Observable} mixin The mixin if being used as a mixin.
        ...
        * handlers at once.
        *
        * **DOM events from CLI Framework {@link CLI.Component Components}**
        *
        * While _some_ CLI Framework Component classes export selected DOM events (e.g. "click", "mouseover" etc), this is usually
        * only done when extra value can be added. For example the {@link CLI.view.View DataView}'s **`{@link
        * CLI.view.View#itemclick itemclick}`** event passing the node clicked on. To access DOM events directly from a
        ...
        */
        ...
        _addDeclaredListeners: function(listeners) {
            ...
            if (listeners instanceof Array) {
                CLI.each(listeners, me._addDeclaredListeners, me);
            } else {
        ...
        /**
        * Adds listeners to any Observable object (or CLI.Element) which are automatically removed when this Component is
        * destroyed.
        *
        * @param {CLI.util.Observable/CLI.dom.Element} item The item to which to add a listener/listeners.
        ...
        * {@link CLI.util.Observable#addListener addListener} options.
        ...
        *
        * @param {CLI.util.Observable/CLI.dom.Element} item The item from which to remove a listener/listeners.
        * @param {Object/String} ename The event name, or an object containing event name properties.
        * @param {Function} fn (optional) If the `ename` parameter was an event name, this is the handler function.
        * @param {Object} scope (optional) If the `ename` parameter was an event name, this is the scope (`this` reference)
        * in which the handler function is executed.
        ...
        * to {@link #addListener}).
        *
        * An event may be set to bubble up an Observable parent hierarchy (See {@link CLI.Component#getBubbleTarget}) by
        ...
        * @private
        * @return {CLI.util.Observable} The bubble parent. null is returned if no bubble target exists
        */
        ...
        * given to {@link CLI.util.Observable#fireEvent} plus the `options` parameter described below.
        *
        ...
        *   Causes the handler to be scheduled to run in an {@link CLI.util.DelayedTask} delayed
        ...
        *   of arguments passed.
        *
        * @param {CLI.util.Observable} options.target
        *   Only call the handler if the event was fired on the target Observable, _not_ if the event
        *   was bubbled up from a child Observable.
        *
        * @param {String} options.element
        *   **This option is only valid for listeners bound to {@link CLI.Component Components}.**
        *   The name of a Component property which references an element to add a listener to.
        *
        *   This option is useful during Component construction to add DOM event listeners to elements of
        *   {@link CLI.Component Components} which will exist only after the Component is rendered.
        ...
        addListener: function(ename, fn, scope, options, caller) {
                ...
                for (ename in options) {
                    ...
                    if (!me.eventOptionsRe.test(ename)) {
                        ...
                        if (!config || (typeof config !== 'function' && !config.fn)) {
                            CLI.Error.raise('No function passed for event ' + me.$className + '.' + ename);
                        }
                        //</debug>
                        */
            ...
            // String, function passed
                ...
                if (event && event.isEvent) {
                    ...
                    me.events[ename] = event = new CLI.util.Event(me, ename);
                }
        ...
        * {@link CLI.util.Observable#addListener} call.**
        * @param {Object} scope (optional) The scope originally specified for the handler. It must be the same as the
        * scope argument specified in the original call to {@link CLI.util.Observable#addListener} or the listener will not be removed.
        */
        ...
        */
        ...
        purgeListeners : function() {
            if (CLI.global.console) {
                CLI.global.console.warn('Observable: purgeListeners has been deprecated. Please use clearListeners.');
            }
        ...
        */
        removeManagedListenerItem: function(isClear, managedListener, item, ename, fn, scope){
            if (isClear || (managedListener.item === item && managedListener.ename === ename && (!fn || managedListener.fn === fn) && (!scope || managedListener.scope === scope))) {
                ...
                if (!isClear) {
                    CLI.Array.remove(this.managedListeners, managedListener);
                }
            }
        },
        ...
        purgeManagedListeners : function() {
            if (CLI.global.console) {
                CLI.global.console.warn('Observable: purgeManagedListeners has been deprecated. Please use clearManagedListeners.');
            }
        ...
        resolveMethod: function (fn, scope) {
            //<debug>
            if (!fn) {
                CLI.Error.raise('No function specified');
            }
            
            if (!scope || !CLI.isObject(scope)) {
                CLI.Error.raise('Named method "' + fn + '" requires a scope object');
            }
            if (!CLI.isFunction(scope[fn])) {
                CLI.Error.raise('No method named "' + fn + '" on ' +
        ...
        */
        ...
        suspendEvent: function() {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (!event || !event.isEvent) {
                    events[ename] = event = new CLI.util.Event(me, ename);
                }
                event.suspend();
            }
        },
        ...
        *
        *     CLI.destroy(this.storeRelayers);
        *
        * or
        ...
        *
        * This is commonly used by CLI.Components to bubble events to owner Containers.
        * See {@link CLI.Component#getBubbleTarget}. The default implementation in CLI.Component returns the
        * Component's immediate owner. But if a known target is required, this can be overridden to access the
        * required target more quickly.
        *
        * Example:
        *
        *     CLI.define('CLI.overrides.form.field.Base', {
        *         override: 'CLI.form.field.Base',
        *
        ...
        *     });
        *
        *     var myForm = CLI.create('CLI.form.Panel', {
        *         title: 'User Details',
        *         items: [{
        *             ...
        *         }],
        ...
        prepareMixin = function (T) {
            if (!T.HasListeners) {
                ...
                T.onExtended(function (U, data) {
                    //<debug>
                    CLI.classSystemMonitor && CLI.classSystemMonitor('extend mixin', arguments);
                    //</debug>
    ...
    Observable.createAlias({
        /**
         * @method
         * @inheritdoc CLI.util.Observable#addListener
         */
        on: 'addListener',
        /**
         * @method
         * Shorthand for {@link #removeListener}.
         * @inheritdoc CLI.util.Observable#removeListener
         */
        un: 'removeListener',
        /**
         * @method
         * Shorthand for {@link #addManagedListener}.
         * @inheritdoc CLI.util.Observable#addManagedListener
         */
        mon: 'addManagedListener',
        /**
         * @method
         * Shorthand for {@link #removeManagedListener}.
         * @inheritdoc CLI.util.Observable#removeManagedListener
         */
    ...
    CLI.apply(proto, {
        ...
        removeMethodListener: function(method, fn, scope){
            ...
            for(i = 0, len = e.before.length; i < len; i++){
                if(e.before[i].fn == fn && e.before[i].scope == scope){
                    CLI.Array.erase(e.before, i, 1);
                    return;
                }
            }
            for(i = 0, len = e.after.length; i < len; i++){
                if(e.after[i].fn == fn && e.after[i].scope == scope){
                    CLI.Array.erase(e.after, i, 1);
                    return;
                }
            }
        },
        ...
        toggleEventLogging: function(toggle) {
            CLI.util.Observable[toggle ? 'capture' : 'releaseCapture'](this, function(en) {
                if (CLI.isDefined(CLI.global.console)) {
                    CLI.global.console.log(en, arguments);
                }
            });
        }
    });
