 ...
 * @private
 */
CLI.define('CLI.util.Event', function() {
  var arraySlice = Array.prototype.slice,
      arrayInsert = CLI.Array.insert,
      toArray = CLI.Array.toArray,
      fireArgs = {};

  return {
    requires: 'CLI.util.DelayedTask',
    ...
    addListener: function(fn, scope, options, caller) {
        ...
        if (!fn) {
            CLI.Error.raise({
                sourceClass: CLI.getClassName(this.observable),
    ...
    createListener: function(fn, scope, o, caller) {
        var me = this,
            namedScope = (scope in CLI._namedScopes),
            listener = {
                fn: fn,
                scope: scope,
                ev: me,
    ...
    findListener: function(fn, scope) {
        ...
        while (i--) {
            ...
            if (listener) {
                ...
                // Otherwise, if the Observable itself adds CLI.emptyFn as a listener, and then CLI.emptyFn is added under another scope, there will be a false match.
    ...
    removeListener: function(fn, scope) {
        ...
        if (fn.fireFn) {
            index = CLI.Array.indexOf(listeners, fn);
        } else {
        ...
        if (index != -1) {
            ...
            // We can use splice directly. The IE8 bug which CLI.Array works around only affects *insertion*
    ...
    getFireInfo: function(listener, forceLookup) {
        ...
        if ((forceLookup || listener.allowLookup) && listener.lateBound) {
            ...
            if (listener.allowLookup) {
                ...
                if (namedScope && namedScope.isController) {
                    ...
                    CLI.Error.raise("Cannot resolve scope 'controller' for '" + listener.ev.name +
        ...
        //<debug>
        if (!fn) {
            CLI.Error.raise('Unable to dynamically resolve method "' + name + '"');
        }
        //</debug>
        return fireArgs;
    },
    ...
    createBuffered: function (handler, listener, o, scope, allowLookup) {
        listener.task = new CLI.util.DelayedTask();
        return function() {
            var fireInfo;
    ...
    createDelayed: function (handler, listener, o, scope, allowLookup) {
        return function() {
            var task = new CLI.util.DelayedTask(),
                fireInfo;
    ...
    createSingle: function (handler, listener, o, scope) {
        return function() {
            ...
            if (event.removeListener(listener.fn, scope) && event.observable) {
                ...
                // event such as CLI Framework's readyEvent): Decrement the listeners count
