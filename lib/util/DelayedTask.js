 ...
 * ## Usage
 * 
 *     var task = new CLI.util.DelayedTask(function(){
 *         alert(CLI.getDom('myInputField').value.length);
 *     });
 *     
 *     // Wait 500ms before calling our function. If the user presses another key
 *     // during that 500ms, it will be cancelled and we'll wait another 500ms.
 *     CLI.get('myInputField').on('keypress', function() {
 *         task.{@link #delay}(500);
 *     });
 * 
 * Note that we are using a DelayedTask here to illustrate a point. The configuration
 * option `buffer` for {@link CLI.util.Observable#addListener addListener/on} will
 ...
 */
CLI.util = CLI.util || {};
CLI.util.DelayedTask = function(fn, scope, args, cancelOnDelay, fireIdleEvent) {
// @define CLI.util.DelayedTask
// @uses CLI.GlobalEvents
    var me = this,
        delay,
        call = function() {
            var globalEvents = CLI.GlobalEvents;
            clearInterval(me.id);
            me.id = null;
            fn.apply(scope, args || []);
