...
var noArgs = [],
            ...
            for (name in members) {
                if (members.hasOwnProperty(name)) {
                    ...
                    if (member && member.$nullFn && privacy !== member.$privacy) {
                        CLI.Error.raise('Cannot use stock function for private method ' +
            ...
            if (!superMethod) {
                ...
                if (!(methodName in parentClass)) {
                    throw new Error("this.callParent() was called but there's no such method (" + methodName +
                                ") found in the parent class (" + (CLI.getClassName(parentClass) || 'Object') + ")");
                }
            }
            //</debug>
    ...
    /**
     * Call the original method that was previously overridden with {@link CLI.Base#override}
     *
     *     CLI.define('My.Cat', {
     *         constructor: function() {
     *             alert("I'm a cat!");
