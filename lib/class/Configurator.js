...
var CLIConfig = CLI.Config,
    configPropMap = CLIConfig.map,
    CLIObject = CLI.Object;

CLI.Configurator = function (cls) {
// @define CLI.class.Configurator
// @define CLI.Configurator
// @require CLI.Config
    ...
    /**
     * @property {CLI.Class} cls The class to which this instance is associated.
     * @private
     * @readonly
     */
    me.cls = cls;
    ...
    if (zuper) {
        /**
         * This object holds an `CLI.Config` value for each config property keyed by name.
         ...
         * @readonly
         */
        me.configs = CLIObject.chain(zuper.configs);
         ...
         * @readonly
         */
        me.cachedConfigs = CLIObject.chain(zuper.cachedConfigs);
         ...
         * @readonly
         */
        me.initMap = CLIObject.chain(zuper.initMap);
         ...
         * @readonly
         */
        me.values = CLIObject.chain(zuper.values);
    } else {
        me.configs = {};
        me.cachedConfigs = {};
        me.initMap = {};
...
CLI.Configurator.prototype = {
    self: CLI.Configurator,
        ...
        for (name in config) {
            ...
            if (cfg) {
                ...
                if (!mixinClass && isCached && !cachedConfigs[name]) {
                    CLI.Error.raise('Redefining config as cached: ' + name + ' in class: ' + Cls.$className);
                }
                //</debug>
                ...
                if (merge) {
                    ...
                    if (currentValue && currentValue.constructor === Object) {
                        ...
                        // need to be modified.
                        value = CLIObject.merge({}, currentValue, value);
                    }
            ...
            if (meta) {
                if (cfg.owner !== Cls) {
                    configs[name] = cfg = CLI.Object.chain(cfg);
                    cfg.owner = Cls;
                }
                CLI.apply(cfg, meta);
                delete cfg.$value;
    ...
    configure: function (instance, instanceConfig) {
        var me = this,
            ...
            values = CLIObject.fork(me.values),
        ...
        //
        // The merging in this loop cannot be completed by CLI.merge(), since we do NOT want to merge
                ...
                if (!cfg) {
                    ...
                    if (merge) {
                        ...
                        if (valuesKey && valuesKey.constructor === Object) {
                            value = CLIObject.merge(values[name], value);
                        } else {
                            value = CLI.clone(value);
                        }
                    }
                }
                values[name] = value;
            ...
            if (cfg) {
                ...
                if (merge) {
                    ...
                    if (baseValue && baseValue.constructor === Object) {
                        value = CLI.Object.merge(baseValue, value);
                    } else {
                        value = CLI.clone(value);
                    }
                }
    ...
    reconfigure: function (instance, instanceConfig, options) {
        ...
        for (name in instanceConfig) {
            ...
            if (cfg) {
                ...
                //<debug>
                if (name !== 'type') {
                    CLI.log.error('No such config "' + name + '" for class ' +
                                  instance.$className);
                }
                //</debug>
                continue;
        ...
        for (i = 0, len = configList.length; i < len; i++) {
            ...
            if (cfg) {
                ...
                cfg = configPropMap[name] || CLI.Config.get(name);
                names = cfg.names;
                ...
                //<debug>
                    ...
                    CLI.Error.raise('Config "' + name + '" has no setter on class ' +
                                    instance.$className);
                }
                //</debug>
            }
