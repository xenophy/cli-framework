 ...
 *      });
 *
 * This uses the cached `CLI.Config` instance for the "foo" property.
 *
 ...
 *      });
 *
 * This creates a prototype chain to the cached "foo" instance of `CLI.Config` and applies
 * the `lazy` option to that new instance. This chained instance is then kept by the
 * `CLI.Configurator` for that class.
 * @private
 */
...
CLI.Config = function (name) {
// @define CLI.class.Config
// @define CLI.Config
...
CLI.Config.get = function (name) {
    var map = CLI.Config.map,
        ret = map[name] || (map[name] = new CLI.Config(name));

    return ret;
};

CLI.Config.prototype = {
    self: CLI.Config,
     ...
     * will be applied.
     *
     * @cfg {CLI.Class} merge.mixinClass The mixin providing the `newValue` or `null` if
    ...
    mergeSets: function (newValue, oldValue, preserveExisting) {
        var ret = oldValue ? CLI.Object.chain(oldValue) : {},
            i, val;
