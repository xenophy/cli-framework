 ...
 * @private
 */
CLI.define('CLI.event.Controller', {
    ...
    doFire: function() {
        ...
        if (observable && observable.isElement) {
            ...
            // In this case you will get an 'CLI.Element' as the first parameter to the listener
        ...
        for (i = 0,ln = firingListeners.length; i < ln; i++) {
            ...
            if (event) {
                ...
                // In Ext4 CLI.EventObject was a singleton event object that was reused as events
                // were fired.  Set CLI.EventObject to the last fired event for compatibility.
                CLI.EventObject = firingArguments[0];
            ...
            if (!firingFn) {
                ...
                if (options.delay) {
                    ...
                    firingFn = CLI.Function.createDelayed(firingFn, options.delay, scope);

                } else if (options.buffer) {

                    firingFn = CLI.Function.createBuffered(firingFn, options.buffer, scope);

                } else if (options.onFrame) {

                    firingFn = CLI.Function.createAnimationFrame(firingFn, scope);
            ...
            if (options.delegate && me.info.targetType === 'element') {
                ...
                firingArguments[1] = CLI.fly(arg1).findParent(info.target, arg1);
            ...
            if (options.single) {
                ...
                me.dispatcher.removeListener(info.targetType, info.target, info.eventName, fn, scope, options, listener.order, listener.observable);
            ...
            if (beforeFn) {
                ...
                result = beforeFn.apply(CLI.global, args) !== false;
