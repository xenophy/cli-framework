 ...
 * @private
 */
CLI.define('CLI.event.Dispatcher', {

    // {{{ requires

    requires: [
        'CLI.event.ListenerStack',
        'CLI.event.Controller'
    ],
    ...
    doGetListenerStack: function(listenerStacks, targetType, target, eventName, createIfNotExist) {
        ...
        //    element: {
        //        '#someId': {
        //            click: new CLI.event.ListenerStack()
        //        }
        //    },
        //    component: {
        //
        ...
        if (!listenerStack) {
            ...
            if (createIfNotExist) {
                ...
                map[eventName] = listenerStack = new CLI.event.ListenerStack();
    ...
    getController: function(targetType, target, eventName, connectedController) {
        ...
        if (!controller) {
            me.controller = controller = new CLI.event.Controller(me);
        }

        if (controller.isFiring) {
            controller = new CLI.event.Controller(me);
        }
    ...
    getHasListeners: function (type, observable) {
        ...
        if (!ret) {
            ...
            ret = has[type] || (has[type] = CLI.Object.chain(this.baseHasListeners));

            if (observable) {

                observable.hasListeners = ret = CLI.Object.chain(ret);
    ...
    doDispatchEvent: function(targetType, target, eventName, args, action, connectedController, capture) {
        ...
        var listenerStack = capture ? this.getCaptureListenerStack(targetType, target, eventName) : this.getListenerStack(targetType, target, eventName, capture),
