...
describe("CLI.Date", function() {
    ...
    describe('Elapsed time between dates', function () {
        ...
        it("should get time elapsed in millisecond between date instantiation", function () {
            ...
            expect(CLI.Date.getElapsed(dateA)).toEqual(3);
        });
        ...
        it("should get time elapsed in millisecond between two dates", function () {
            ...
            expect(CLI.Date.getElapsed(dateA, dateB)).toEqual(19);
        });
    });
    ...
    describe("getShortMonthName", function() {
       it("should return 3 letter abbreviation for the corresponding month [0-11]", function() {
           expect(CLI.Date.getShortMonthName(0)).toBe("Jan");
           expect(CLI.Date.getShortMonthName(1)).toBe("Feb");
           expect(CLI.Date.getShortMonthName(2)).toBe("Mar");
           expect(CLI.Date.getShortMonthName(3)).toBe("Apr");
           expect(CLI.Date.getShortMonthName(4)).toBe("May");
           expect(CLI.Date.getShortMonthName(5)).toBe("Jun");
           expect(CLI.Date.getShortMonthName(6)).toBe("Jul");
           expect(CLI.Date.getShortMonthName(7)).toBe("Aug");
           expect(CLI.Date.getShortMonthName(8)).toBe("Sep");
           expect(CLI.Date.getShortMonthName(9)).toBe("Oct");
           expect(CLI.Date.getShortMonthName(10)).toBe("Nov");
           expect(CLI.Date.getShortMonthName(11)).toBe("Dec");
       });
    });

    describe("getShortDayName", function() {
       it("should return 3 letter abbreviation for the corresponding weekday [0-6]", function() {
          expect(CLI.Date.getShortDayName(0)).toBe("Sun");
          expect(CLI.Date.getShortDayName(1)).toBe("Mon");
          expect(CLI.Date.getShortDayName(2)).toBe("Tue");
          expect(CLI.Date.getShortDayName(3)).toBe("Wed");
          expect(CLI.Date.getShortDayName(4)).toBe("Thu");
          expect(CLI.Date.getShortDayName(5)).toBe("Fri");
          expect(CLI.Date.getShortDayName(6)).toBe("Sat");
    ...
    describe("getMonthNumber", function() {
        it("should return the month number [0-11] for the corresponding short month name", function() {
           expect(CLI.Date.getMonthNumber("jan")).toBe(0);
           expect(CLI.Date.getMonthNumber("feb")).toBe(1);
           expect(CLI.Date.getMonthNumber("mar")).toBe(2);
           expect(CLI.Date.getMonthNumber("apr")).toBe(3);
           expect(CLI.Date.getMonthNumber("MAY")).toBe(4);
           expect(CLI.Date.getMonthNumber("JUN")).toBe(5);
           expect(CLI.Date.getMonthNumber("JUL")).toBe(6);
           expect(CLI.Date.getMonthNumber("AUG")).toBe(7);
           expect(CLI.Date.getMonthNumber("Sep")).toBe(8);
           expect(CLI.Date.getMonthNumber("Oct")).toBe(9);
           expect(CLI.Date.getMonthNumber("Nov")).toBe(10);
           expect(CLI.Date.getMonthNumber("Dec")).toBe(11);
        });
        it("should return the month number [0-11] for the corresponding full month name", function() {
            expect(CLI.Date.getMonthNumber("january")).toBe(0);
            expect(CLI.Date.getMonthNumber("february")).toBe(1);
            expect(CLI.Date.getMonthNumber("march")).toBe(2);
            expect(CLI.Date.getMonthNumber("april")).toBe(3);
            expect(CLI.Date.getMonthNumber("MAY")).toBe(4);
            expect(CLI.Date.getMonthNumber("JUNE")).toBe(5);
            expect(CLI.Date.getMonthNumber("JULY")).toBe(6);
            expect(CLI.Date.getMonthNumber("AUGUST")).toBe(7);
            expect(CLI.Date.getMonthNumber("September")).toBe(8);
            expect(CLI.Date.getMonthNumber("October")).toBe(9);
            expect(CLI.Date.getMonthNumber("November")).toBe(10);
            expect(CLI.Date.getMonthNumber("December")).toBe(11);
        });
    });

    describe("formatContainsHourInfo", function() {
       it("should return true when format contains hour info", function() {
          expect(CLI.Date.formatContainsHourInfo("d/m/Y H:i:s")).toBeTruthy();
       });
       it("should return false when format doesn't contains hour info", function() {
          expect(CLI.Date.formatContainsHourInfo("d/m/Y")).toBeFalsy();
       });
    });

    describe("formatContainsDateInfo", function() {
        it("should return true when format contains date info", function() {
            expect(CLI.Date.formatContainsDateInfo("d/m/Y H:i:s")).toBeTruthy();
        });
        it("should return false when format doesn't contains date info", function() {
            expect(CLI.Date.formatContainsDateInfo("H:i:s")).toBeFalsy();
        });
    });

    describe("isValid", function() {
        it("should return true for valid dates", function() {
           expect(CLI.Date.isValid(1981, 10, 15, 16, 30, 1, 2)).toBeTruthy();
        });
        it("should return false for invalid dates", function() {
            expect(CLI.Date.isValid(999999, 10, 15, 16, 30, 1, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 13, 15, 16, 30, 1, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 10, 32, 16, 30, 1, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 10, 15, 25, 30, 1, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 10, 15, 16, 60, 1, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 10, 15, 16, 30, 60, 2)).toBeFalsy();
            expect(CLI.Date.isValid(1981, 10, 15, 16, 30, 1, 100000)).toBeFalsy();
        });
    });
    ...
    describe("parse", function() {
        ...
        it("should return null when parsing an invalid date like Feb 31st in strict mode", function() {
           expect(CLI.Date.parse("2011-02-31", "Y-m-d", true)).toBeNull();
        });
       ...
       describe("week/year", function() {
           ...
           it("should always be a Monday", function() {
               ...
               for (i = 2012; i <= 2020; ++i) {
                   for (j = 1; j < 53; ++j) {
                       expect(CLI.Date.parse(i + '-' + CLI.String.leftPad(j, 2, '0'), 'Y-W').getDay()).toBe(1);
                   }
               }
           });
       });
    ...
    describe("isEqual", function() {
        it("should return true if both dates are exactly the same", function() {
            ...
            expect(CLI.Date.isEqual(date1, date2)).toBeTruthy();
        });
        it("should return true if there is at least 1 millisecond difference between both dates", function() {
            ...
            expect(CLI.Date.isEqual(date1, date2)).toBeFalsy();
        });
        it("should return false if one one of the dates is null/undefined", function() {
           expect(CLI.Date.isEqual(new Date(), undefined)).toBeFalsy();
           expect(CLI.Date.isEqual(new Date(), null)).toBeFalsy();
           expect(CLI.Date.isEqual(undefined, new Date())).toBeFalsy();
           expect(CLI.Date.isEqual(null, new Date())).toBeFalsy();
        });
        it("should return true if both dates are null/undefined", function() {
           expect(CLI.Date.isEqual(null, null)).toBeTruthy();
           expect(CLI.Date.isEqual(null, undefined)).toBeTruthy();
           expect(CLI.Date.isEqual(undefined, null)).toBeTruthy();
           expect(CLI.Date.isEqual(undefined, undefined)).toBeTruthy();
        });
    });

    describe("getDayOfYear", function() {
       it("should return the day of year between 0 and 364 for non-leap years", function() {
           expect(CLI.Date.getDayOfYear(new Date(2001, 0, 1))).toBe(0);
           expect(CLI.Date.getDayOfYear(new Date(2001, 11, 31))).toBe(364);
       });
       it("should return the day of year between 0 and 365 for leap years", function() {
           expect(CLI.Date.getDayOfYear(new Date(2000, 0, 1))).toBe(0);
           expect(CLI.Date.getDayOfYear(new Date(2000, 11, 31))).toBe(365);
       });
    });

    describe("getFirstDayOfMonth", function() {
       it("should return the number [0-6] of the first day of month of the given date", function() {
           expect(CLI.Date.getFirstDayOfMonth(new Date(2007, 0, 1))).toBe(1);
           expect(CLI.Date.getFirstDayOfMonth(new Date(2000, 0, 2))).toBe(6);
           expect(CLI.Date.getFirstDayOfMonth(new Date(2011, 0, 3))).toBe(6);
           expect(CLI.Date.getFirstDayOfMonth(new Date(2011, 6, 4))).toBe(5);
           expect(CLI.Date.getFirstDayOfMonth(new Date(2011, 11, 5))).toBe(4);
       });
    });

    describe("getLastDayOfMonth", function() {
        it("should return the number [0-6] of the last day of month of the given date", function() {
            expect(CLI.Date.getLastDayOfMonth(new Date(2007, 0, 1))).toBe(3);
            expect(CLI.Date.getLastDayOfMonth(new Date(2000, 0, 2))).toBe(1);
            expect(CLI.Date.getLastDayOfMonth(new Date(2011, 0, 3))).toBe(1);
            expect(CLI.Date.getLastDayOfMonth(new Date(2011, 6, 4))).toBe(0);
            expect(CLI.Date.getLastDayOfMonth(new Date(2011, 11, 5))).toBe(6);
        });
    });
    ...
    describe("getFirstDateOfMonth", function() {
        it("should return the date corresponding to the first day of month of the given date", function() {
            expect(CLI.Date.getFirstDateOfMonth(new Date(2007, 0, 1))).toEqual(new Date(2007, 0, 1));
            expect(CLI.Date.getFirstDateOfMonth(new Date(2000, 0, 2))).toEqual(new Date(2000, 0, 1));
            expect(CLI.Date.getFirstDateOfMonth(new Date(2011, 0, 3))).toEqual(new Date(2011, 0, 1));
            expect(CLI.Date.getFirstDateOfMonth(new Date(2011, 6, 4))).toEqual(new Date(2011, 6, 1));
            expect(CLI.Date.getFirstDateOfMonth(new Date(2011, 11, 5))).toEqual(new Date(2011, 11, 1));
        });
    });
    ...
    describe("getLastDateOfMonth", function() {
        it("should return the date corresponding to the last day of month of the given date", function() {
            expect(CLI.Date.getLastDateOfMonth(new Date(2007, 1, 1))).toEqual(new Date(2007, 1, 28));
            expect(CLI.Date.getLastDateOfMonth(new Date(2000, 1, 2))).toEqual(new Date(2000, 1, 29));
            expect(CLI.Date.getLastDateOfMonth(new Date(2011, 0, 3))).toEqual(new Date(2011, 0, 31));
            expect(CLI.Date.getLastDateOfMonth(new Date(2011, 5, 4))).toEqual(new Date(2011, 5, 30));
            expect(CLI.Date.getLastDateOfMonth(new Date(2011, 11, 5))).toEqual(new Date(2011, 11, 31));
        });
    });

    describe("getSuffix", function() {
       it("should return st for 1, 21 and 31", function() {
          expect(CLI.Date.getSuffix(new Date(2011, 0, 1))).toBe("st");
          expect(CLI.Date.getSuffix(new Date(2011, 0, 21))).toBe("st");
          expect(CLI.Date.getSuffix(new Date(2011, 0, 31))).toBe("st");
       });
       it("should return nd for 2 and, 22", function() {
           expect(CLI.Date.getSuffix(new Date(2011, 0, 2))).toBe("nd");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 22))).toBe("nd");
       });
       it("should return rd for 3 and, 23", function() {
           expect(CLI.Date.getSuffix(new Date(2011, 0, 3))).toBe("rd");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 23))).toBe("rd");
       });
       it("should return th for days [11-13] and days ending in [4-0]", function() {
           expect(CLI.Date.getSuffix(new Date(2011, 0, 4))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 5))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 6))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 7))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 8))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 9))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 10))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 11))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 12))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 13))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 14))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 15))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 16))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 17))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 18))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 19))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 20))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 24))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 25))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 26))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 27))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 28))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 29))).toBe("th");
           expect(CLI.Date.getSuffix(new Date(2011, 0, 30))).toBe("th");
       });
    });
    ...
    describe("isDST", function() {
        ...
        xit("should return true from the end of March till the middle of October", function() {
            expect(CLI.Date.isDST(new Date(2010, 2, 31))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 3, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 4, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 5, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 6, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 7, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 8, 15))).toBeTruthy();
            expect(CLI.Date.isDST(new Date(2010, 9, 15))).toBeTruthy();
        });

        it("should return false from the middle of November till the start of March", function() {
            expect(CLI.Date.isDST(new Date(2010, 10, 15))).toBeFalsy();
            expect(CLI.Date.isDST(new Date(2010, 11, 15))).toBeFalsy();
            expect(CLI.Date.isDST(new Date(2010, 0, 15))).toBeFalsy();
            expect(CLI.Date.isDST(new Date(2010, 1, 15))).toBeFalsy();
            expect(CLI.Date.isDST(new Date(2010, 2, 1))).toBeFalsy();
       });
    });
    ...
    describe("add", function() {
        ...
        it("should add milliseconds", function() {
            expect(CLI.Date.add(date, CLI.Date.MILLI, 1)).toEqual(new Date(2000, 0, 1, 0, 0, 0, 1));
        });
        it("should add seconds", function() {
            expect(CLI.Date.add(date, CLI.Date.SECOND, 1)).toEqual(new Date(2000, 0, 1, 0, 0, 1, 0));
        });
        it("should add minutes", function() {
            expect(CLI.Date.add(date, CLI.Date.MINUTE, 1)).toEqual(new Date(2000, 0, 1, 0, 1, 0, 0));
        });
        it("should add hours", function() {
            expect(CLI.Date.add(date, CLI.Date.HOUR, 1)).toEqual(new Date(2000, 0, 1, 1, 0, 0, 0));
        });
        it("should add days", function() {
            expect(CLI.Date.add(date, CLI.Date.DAY, 1)).toEqual(new Date(2000, 0, 2, 0, 0, 0, 0));
        });
        it("should add months", function() {
            expect(CLI.Date.add(date, CLI.Date.MONTH, 1)).toEqual(new Date(2000, 1, 1, 0, 0, 0, 0));
        });
        it("should add years", function() {
           expect(CLI.Date.add(date, CLI.Date.YEAR, 1)).toEqual(new Date(2001, 0, 1, 0, 0, 0, 0));
        });
        it("should consider last day of month when adding months", function() {
           expect(CLI.Date.add(new Date(2001, 0, 29), CLI.Date.MONTH, 1)).toEqual(new Date(2001, 1, 28));
           expect(CLI.Date.add(new Date(2001, 0, 30), CLI.Date.MONTH, 1)).toEqual(new Date(2001, 1, 28));
           expect(CLI.Date.add(new Date(2001, 0, 31), CLI.Date.MONTH, 1)).toEqual(new Date(2001, 1, 28));
           expect(CLI.Date.add(new Date(2000, 0, 29), CLI.Date.MONTH, 1)).toEqual(new Date(2000, 1, 29));
           expect(CLI.Date.add(new Date(2000, 0, 30), CLI.Date.MONTH, 1)).toEqual(new Date(2000, 1, 29));
           expect(CLI.Date.add(new Date(2000, 0, 31), CLI.Date.MONTH, 1)).toEqual(new Date(2000, 1, 29));
        });
        it("should consider last day of month when adding years", function() {
            expect(CLI.Date.add(new Date(2000, 1, 29), CLI.Date.YEAR, 1)).toEqual(new Date(2001, 1, 28));
        });
    });
    ...
    describe("between", function() {
        ...
        it("should return true if the date is equal to the start date", function() {
            expect(CLI.Date.between(new Date(2000, 0, 1), startDate, endDate)).toBeTruthy();
        });
        it("should return true if the date is equal to the end date", function() {
            expect(CLI.Date.between(new Date(2000, 0, 31), startDate, endDate)).toBeTruthy();
        });
        it("should return true if date is between start and end dates", function() {
            expect(CLI.Date.between(new Date(2000, 0, 15), startDate, endDate)).toBeTruthy();
        });
        it("should return false if date is before start date", function() {
            expect(CLI.Date.between(new Date(1999, 11, 31, 23, 59, 59), startDate, endDate)).toBeFalsy();
        });
        it("should return false if date is after end date", function() {
            expect(CLI.Date.between(new Date(2000, 0, 31, 0, 0, 1), startDate, endDate)).toBeFalsy();
        });
    });
